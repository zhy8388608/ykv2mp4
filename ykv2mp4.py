...
def ykv2mp4(input_file, output_file):
    ...
    with open(txt_path, "wb") as out_f:
        for i in range(1, count + 1):
            out_f.write(
                f"file '{os.path.join(temp_folder, f'{i}.{type}')}'\n".encode("utf-8")
            )
        ...
        if entry.is_file():
            if entry.name.endswith(".ykv"):
                ...
                output_file = os.path.join(
                    output_folder, entry.name.replace(".ykv", ".mp4")
                )
...
def main():
    if len(sys.argv) < 2:
        print(
            f"Usage1: {script_path} <input_file> ...\nUsage2: {script_path} <input_folder> <output_folder>"
        )
        os.system("pause")
        return
